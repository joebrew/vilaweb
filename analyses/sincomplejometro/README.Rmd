...
ggplot(data = pd,
  ...
  labs(x = '',
       ...
       subtitle = 'Ordenat per % de tots els piulets',
       caption = 'Inclou la versió en castellà també: "sin complejos".\nJoe Brew (@joethebrew). VilaWeb (www.vilaweb.cat).')
```
...
ggplot(data = pd,
  ...
  scale_color_manual(name = 'Partit',
                    values = databrew::make_colors(n = length(unique(pd$party)))) +
...
ggplot(data = pdx %>% filter(username %in% keep) %>%
  ...
  scale_y_continuous(breaks = seq(0, 1, 0.25)) +
      labs(x = 'Frase',
       y = '%',
       title = 'Freqüencia de certes paraules en piulets',
       subtitle ='"destruir", "liquidar", "trencar"*, i "sense complexos"**',
       caption = '**Inclou la versió en castellà també: "sin complejos".\n*Inclou la versió en castellà també: "romper".\nJoe Brew (@joethebrew). VilaWeb (www.vilaweb.cat).') +
