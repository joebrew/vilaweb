...
#   mutate(xx = axis_super_simple) %>%
#   filter(!is.na(xx)) %>%
#   group_by(source, provincia, xx) %>%
#   tally %>%
#   group_by(source, provincia) %>%
...
#   bind_rows(cosmo %>% mutate(source = 'Cosmo')) %>%
#   group_by(source) %>%
#   summarise(val = mean(monarquia_confianca, na.rm = T))
# pd
# 
#   
# 
...
# pd <- bop %>% mutate(source = 'BOP') %>% filter(date >= '2018-01-01') %>%
#   bind_rows(cosmo %>% mutate(source = 'Cosmo')) %>%
#   mutate(xx = identificacio_simple) %>%
#   filter(!is.na(xx)) %>%
#   group_by(source, xx) %>%
#   tally %>%
#   group_by(source) %>%
