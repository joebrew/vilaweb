---
title: "Quantifying the best and worst tweets among Spanish and Catalan politicians"
...
make_tern_color <- function(language = 'catalan',
  ...
  tl <- tl %>% filter(eix_indepe_unio != 'sobiranista')
  if(language == 'catalan'){
    x = 'Replies'
       y = 'Retweets'
       z = 'Likes'
...
make_tern_color(alpha = 0.05, size = 0.05)
```

Do twitter Ratios really matter? No. But perhaps the issue speaks to an underlying - and only infrequently discussed - subject in Spanish and Catalan politics: _who cares more?_ 

In cases of gridlock or 50-50 splits, the eventual victor is not necessarily decided by the marginal changes in majorities/minorities, but rather by the extent to which the constituency is _passionate_ about the topic. In other words, just because the population is split evenly on an issue does not mean that both sides of the split care evenly about that issue. For example, though the Catalan population is divided on independence, [those in favor of independence are _more interested_ in politics than those opposed](https://twitter.com/joethebrew/status/1066744957363474433).

![](img/4.jpeg)


The level of interest in or passion about a topic matters. And this analysis, as frivolous as it may seem, is consistent with [survey data](https://twitter.com/joethebrew/status/1066030235823554566) in Catlonia showing that unionist politicians are rated more poorly than their independentist counterparts....
...
for(i in 1:length(people)){
  ...
  print(make_tern(data = these_data, show_density = T, title = this_person, alpha = 0.2, size = 0.2))
}
```
