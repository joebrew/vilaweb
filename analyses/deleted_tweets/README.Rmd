---
title: "73.269 piulets esborrats"
...
"Cita aquest tuit amb l'insult en català que mes t'agradi" [va piular l'usuari 'CatalanThings' a la vespra del 3 de gener del 2017](https://twitter.com/catalanthings/status/816360038499581953). Una hora i quatorze minuts més tard, el diputat Gabriel Rufían va citar el piulet afegint ell el texte següent: "Referèndum pactat amb l'estat".
...
Albiol is a prolific tweeter, with `r users %>% filter(tolower(screen_name) == 'albiol_xg') %>% .$statuses_count` tweets on his timeline at the time of writing. But he is also a prolific deleter (`r df %>% filter(username == 'albiol_xg') %>% tally %>% unlist` deleted tweets). His deletion behavior is similar to most people's: he sometimes makes a mistake, and deletes the tweet very soon afterwards (hence the pronounced diagonal line). Most of his deletions are retweets.

```{r}
print(plot_when(user = 'albiol_xg', ca = FALSE))
```
...
```{r}
print(plot_when(user = 'carlossm_cs', ca = FALSE))
```


### Fernando de Páramo

On January 6, 2020, at about 8:00PM, it appears that the former secretary of communication for the Ciutadans party deleted [his twitter account](https://twitter.com/ferdeparamo) entirely, along with `r df %>% filter(username == 'ferdeparamo') %>% nrow` tweets.

```{r}
print(plot_when(user = 'ferdeparamo', ca = FALSE))
```
...
```{r}
print(plot_when(user = 'gabrielrufian', ca = FALSE))
```

### Inés Arrimadas

From 2016 until mid-2017, Inés Arrimadas appears to have used an automated tweet deletion service, which deleted here tweets approximately 3 months after they were written. In fact, all of her tweets prior to July 22, 2017 have been deleted (in total, this makes for `r nrow(df[df$username == 'inesarrimadas',])` deleted tweets). Since then, she now rarely deletes tweets (apart from those which are tweeted and then immediately retweeted).


```{r}
print(plot_when(user = 'inesarrimadas', ca = FALSE))
# inesarrimadasx  = df %>% filter(username == 'inesarrimadas')
```

### Oriol Junqueras

Relative to Arrimadas' `r nrow(df[df$username == 'inesarrimadas',])` deleted tweets, Junqueras deletes very little (only `r nrow(df[df$username == 'junqueras',])`), mostly retweets.

```{r}
print(plot_when(user = 'junqueras', ca = FALSE))
```
...
```{r}
print(plot_when(user = 'krls', ca = FALSE))
```

### Miquel Iceta

Iceta is a very prolific tweeter and retweeter (`r users %>% filter(screen_name == 'miqueliceta') %>% .$statuses_count` tweets!). Which makes it all the more surprising that he has only deleted `r nrow(df[df$username == 'miqueliceta',])`, mostly retweets. It is curious, however, that he appears to regularly go back to mid-2015 and delete retweets from that period.

```{r}
print(plot_when(user = 'miqueliceta', ca = FALSE))
```
...
```{r}
print(plot_when(user = 'miriamnoguerasm', ca = FALSE))
```
...
The horizontal line: May 24, 2017, when he deleted `r df %>% filter(username == 'polgibert', as.Date(deleted_at) == as.Date('2017-05-24')) %>% tally %>% .$n` tweets in one day.

```{r}
print(plot_when(user = 'polgibert', ca = FALSE))
```


### Raúl Moreno

Moreno also appears to have undergone a social network cleanup on May 22, 2019, a day in which he deleted `r df %>% filter(username == 'raulmorenom') %>% filter(as.Date(deleted_at) == '2019-05-22') %>% nrow` tweets.

```{r}
print(plot_when(user = 'raulmorenom', ca = FALSE))
```


### Further analysis

More interesting than _when_ tweets were written and deleted is _what_: that is, what were the deleted tweets about.

Unless you have a spare 610 hours (the time it would take to read all `r nrow(df)` tweets at a rate of 30 seconds per tweet), we'll need to _search_. And manually exploring deleted tweets by myself is, simply, too big of a job.

So, rather than explore the `r nrow(df)` tweets myself, I'll simply hand them over to you, the public. You can explore them through the [politwoops website interface](www.politwoops.eu) if you'd like, but that interface is built around scrolling through specific politicians, rather than looking for specific text. So, to faciliate the latter, here's a simple web application built for quickly searching (for any text) and filtering (for the Spanish/Catalan politicians in this analysis): https://datacat.cc/piulets.
...
```{r}
plot_p_deleted(ca = TRUE)
```


```{r}
print(plot_when(user = 'albiol_xg', ca = TRUE))
```
...
```{r}
print(plot_when(user = 'carlossm_cs', ca = TRUE))
```
...
```{r}
print(plot_when(user = 'ferdeparamo', ca = TRUE))
```



```{r}
print(plot_when(user = 'gabrielrufian', ca = TRUE))
```


```{r}
print(plot_when(user = 'inesarrimadas', ca = TRUE))
# inesarrimadasx  = df %>% filter(username == 'inesarrimadas')
```
...
```{r}
print(plot_when(user = 'junqueras', ca = TRUE))
```
...
```{r}
print(plot_when(user = 'krls', ca = TRUE))
```


```{r}
print(plot_when(user = 'miqueliceta', ca = TRUE))
```



```{r}
print(plot_when(user = 'miriamnoguerasm', ca = TRUE))
```
...
```{r}
print(plot_when(user = 'polgibert', ca = TRUE))
```


```{r}
print(plot_when(user = 'raulmorenom', ca = TRUE))
```
