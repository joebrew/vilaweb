...
mossos_chart2 <- function(ca = TRUE){
  ...
  out_list <- list()
  crime_types <- sort(unique(pd$`Tipus de fet`))
  for(i in 1:length(crime_types)){
    message(i)
    this_crime_type <- crime_types[i]
    the_labs$title <- this_crime_type
    sub_data <- pd %>% filter(`Tipus de fet` == this_crime_type)
    out_list[[i]] <- 
...
combination_data <- function(timey = 'month'){
