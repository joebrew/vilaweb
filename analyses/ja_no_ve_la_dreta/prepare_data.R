...
if(!file_name %in% dir()){
  ...
  save(tl, tl_party, file = file_name)
} else {
  load(file_name)
}
...
make_pre_election_plot <- function(){
  ...
  ggplot(data = pd,
    ...
    labs(title = "Piulets de polítics socialistes esmentant 'la dreta'",
         subtitle = 'Percentatge per mes. 17 diputats del PSC, 14 ministres del PSOE, i Pedro Sánchez',
         caption = paste0(numberfy(nr), " piulets dels 32 socialistes. Freqüència mensual de paraules: 'dreta', 'dretes', 'derecha', 'derechas'.\nGràfic de @joethebrew. www.vilaweb.cat"),
         x = 'Mes') +
...
make_pre_election_plot_person <- function(){
  ...
  ggplot(data = pd,
    ...
    labs(title = "Piulets de polítics socialistes esmentant 'la dreta'",
         ...
         caption = paste0("Freqüència mensual de paraules: 'dreta', 'dretes', 'derecha', 'derechas'.\nGràfic de @joethebrew. www.vilaweb.cat"),
         x = 'Mes',
         y = 'Percentatge') +
    geom_text(aes(label = round(p, digits = 1)),
              nudge_y = 0.5,
...
make_post_election_plot_person <- function(){
  ...
  ggplot(data = pd,
    ...
    scale_x_date(breaks = sort(unique(pd$date)),
                 labels = cat_date(sort(unique(pd$date)))) +
    labs(title = "Piulets de polítics socialistes esmentant 'la dreta'",
         subtitle = 'Percentatge per mes. Línia vermella: eleccions 28-A.',
         caption = paste0("Freqüència mensual de paraules: 'dreta', 'dretes', 'derecha', 'derechas'.\nGràfic de @joethebrew. www.vilaweb.cat"),
         x = 'Mes',
         y = 'Percentatge') +
    geom_text(aes(label = round(p, digits = 1)),
...
make_post_election_plot_party <- function(){
  ...
  ggplot(data = pd,
    ...
    scale_x_date(breaks = sort(unique(pd$date)),
                 labels = cat_date(sort(unique(pd$date)))) +
    labs(title = "Piulets dels comptes oficials PSOE/PSC esmentant 'la dreta'",
         subtitle = 'Percentatge per mes',
         caption = paste0("Freqüència mensual de paraules: 'dreta', 'dretes', 'derecha', 'derechas'.\nGràfic de @joethebrew. www.vilaweb.cat"),
         x = 'Mes',
         y = 'Percentatge') +
...
make_terrorism_plot <- function(){
  ...
  pd <- pd %>%
    mutate(right = 'Dreta') %>%
    bind_rows(pd2 %>% mutate(right = 'Violència'))
  pd <- pd %>%
    dplyr::select(date, username, right, n, name, p) %>%
    dplyr::rename(key = right)
