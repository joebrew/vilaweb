...
if(processed %in% dir()){
  ...
  # Get their tweets
  if('tweets2.RData' %in% dir()){
    load('tweets2.RData')
  } else {
    tl <- get_twitter_data_from_database(people = people)
    save(tl, file = 'tweets2.RData')
  }
...
pd <- tl %>%
  ...
  summarise(tweets = n(),
            ...
            overall_ratio = replies_no_replies / retweets_no_replies,
            plus1 = length(which(ratio[!is_reply] >= 1)) / tweets_no_replies) %>%
  ungroup %>%
